...
\title{SEC-EDGAR-GPT: A GPT-2 (124M) Language Model Trained from Scratch on SEC EDGAR Filings%
...
We collected SEC-EDGAR filings from the kapilrao/SEC-EDGAR dataset on Hugging Face\footnote{\url{https://huggingface.co/datasets/kapilrao/SEC-EDGAR}}, which contains processed EDGAR documents. The training corpus consists of:
...
\subsection{Prompt 1: Healthcare SaaS Business Description}

\textbf{Input:} A cloud-based healthcare enterprise software provider with 2,500 customers, \$487.2M revenue (+28\%), 78\% subscription revenue, and 8,200 employees.

\textbf{Generated continuation (first 500 tokens):}

\begin{Verbatim}[fontsize=\small]
Our business is heavily dependent on the resources of
our clinical trials and the discovery and development
of our product candidates. Our technology costs are
based on the materials, manufacturing, test, and
customer support costs.

We are a global leader in the healthcare industry, and
the care industry is undergoing a number of product
development, including the following:

Our core product development, product development and
commercialization of our product candidates, which we
currently license agreements with third-party
manufacturers. We also license and research and
development activities, including computer-based and
electronic testing services, to enhance the diagnostic,
medical, manufacturing, process, manufacturing, process,
...
Our development of new product candidates, including
the following:

the expansion of new product candidates, including our
product candidates and the existing products, including
new product candidates, and our existing product
candidates;

Our commercialization of new product candidates and
new product candidates, including the following:
\end{Verbatim}

\textbf{Observations:}
\begin{itemize}
    \item Input context about the healthcare SaaS company correctly echoed in first paragraphs
    \item Rapidly drifts to biotech/pharma domain: ``clinical trials,'' ``product candidates,'' ``commercialization''
    \item Enters a loop repeating ``commercialization of our product candidates'' 15+ times
    \item Hallucinates drug names: ``X-Avent therapies,'' ``X-Zentib,'' ``Q-partnerib,'' ``S-Zentib''
    \item Generates fictional drug development pipeline language absent from the input
    \item Training data appears dominated by biotech 10-K filings, causing systematic domain drift
\end{itemize}

\subsection{Prompt 2: Risk Factors}

\textbf{Input:} Risk factor disclosures for a healthcare SaaS company: net losses of \$42.3M--\$89.1M over three years, accumulated deficit of \$523.4M, and quarterly revenue volatility.

\textbf{Generated continuation (first 500 tokens):}

\begin{Verbatim}[fontsize=\small]
We have financed and expect to continue to incur
losses and we may not achieve profitability in the
future.

We have incurred significant losses since inception
and expect our net losses since inception and expect
to continue to incur losses and we may never achieve
profitability through the development and
commercialization of our product candidates. Many of
our product candidates are in development, and we have
not yet generated any revenue from product sales. We
have experienced significant operating losses since
inception and expect to continue to incur losses on
our own and through the sale of common stock in the
future.

We have incurred significant net losses since inception
and have incurred negative opinion and our operating
losses since inception.

We continue to develop and market new products and
technologies in the development and commercialization
of our product candidates, and as a result, our
revenues from product sales and marketing, are subject
to product commercialization and regulatory
requirements. We cannot be certain that we will ever
achieve profitability through sales of product
candidates, if ever, and as a result, we will never
achieve profitability in the future.

Our future success depends upon our ability to develop
and commercialize our product candidates. We intend to
create new technologies that are the subject of
technological change.
\end{Verbatim}

\textbf{Observations:}
\begin{itemize}
    \item Input risk factors (net losses, accumulated deficit) echoed with correct figures
    \item Continuation maintains authentic SEC risk factor tone and legal hedging language
    \item Loops on ``product candidates'' and ``commercialization'' similar to Prompt 1
    \item Self-contradictory statements: ``may not achieve profitability'' followed by ``will never achieve profitability''
    \item Strong boilerplate generation but unable to introduce novel risk scenarios
    \item Grammatically correct even when semantically repetitive
\end{itemize}
...
We serve the trained model through an OpenAI-compatible API server. A key design decision was whether to load the checkpoint using the native nanoGPT \texttt{model.py} or to convert weights to Hugging Face format and serve via \texttt{transformers.GPT2LMHeadModel}. We chose native serving and argue this is the correct choice for a self-trained nanoGPT model.

\subsection{The \texttt{bias=False} Conversion Problem}

nanoGPT's architecture makes bias optional everywhere via a configuration flag:
...
We trained with \texttt{bias=False}, so our checkpoint contains \textbf{no} LayerNorm bias parameters and \textbf{no} Linear bias parameters. Hugging Face's \texttt{GPT2LMHeadModel} has no such switch---GPT-2's architecture always includes biases (LayerNorm $\beta$, attention and MLP \texttt{c\_proj} bias terms). Converting nanoGPT to HF format requires fabricating zero-filled bias tensors to fill slots the training never used.

While mathematically a zero bias is a no-op ($xW + 0 = xW$), the real risk lies in the conversion mapping itself. nanoGPT stores attention as a single fused \texttt{c\_attn} weight ($3072 \times 1024$ for Q, K, V) while HF expects separate \texttt{Conv1D} projections with transposed weight layouts. Any off-by-transpose error or incorrect weight tying in the converter silently degrades output without crashing---producing elevated loss or gibberish that is difficult to diagnose. Serving native eliminates the entire conversion surface.

\subsection{Sampling Parity}

nanoGPT's \texttt{generate()} function is approximately 10 lines with transparent semantics: it applies temperature scaling, optional top-k filtering, and multinomial sampling. Hugging Face's \texttt{model.generate()} is a dispatcher with its own \texttt{top\_k}, \texttt{temperature}, \texttt{do\_sample}, and \texttt{pad\_token} semantics spanning thousands of lines. Matching nanoGPT's evaluation-time behavior through the HF API requires reverse-engineering which parameters map to what. Native serving uses the identical code path used during training and evaluation, guaranteeing sampling parity.

\subsection{Dependency Overhead}

Native serving requires only \texttt{torch} and \texttt{tiktoken}, eliminating the \texttt{transformers} dependency and its transitive requirements (\texttt{safetensors}, \texttt{tokenizers}, \texttt{huggingface\_hub}). This reduces cold start time and simplifies deployment on constrained environments such as cloud GPU instances.

\subsection{When Hugging Face or vLLM Would Be Preferred}

For completeness, HF \texttt{transformers} (or better, vLLM) would be the right choice when: (a) batched, high-throughput serving with continuous batching and paged KV cache is required---nanoGPT's \texttt{generate()} is single-sequence with no KV cache, recomputing the full prefix every token at $O(T^2)$ cost; (b) the ecosystem features are needed---streaming, beam search, logit processors, quantization (bitsandbytes/GPTQ), or \texttt{device\_map} sharding; or (c) the model is being handed to external users who expect a standard \texttt{from\_pretrained} interface. For a 124M demonstration model, none of these apply.
...
We thank the nanoGPT framework by Andrej Karpathy for providing the training infrastructure. The SEC-EDGAR data was sourced from the kapilrao/SEC-EDGAR dataset on Hugging Face (\url{https://huggingface.co/datasets/kapilrao/SEC-EDGAR}). Training was performed on consumer hardware (NVIDIA RTX 4070) to demonstrate accessibility of language model training. For helpful discussions, we thank Ming Jian Wei, Du Chun, and Parjanya Mudunuri.
